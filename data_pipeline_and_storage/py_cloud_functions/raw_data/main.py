...
def process_option_array(option_array_data):
    ...
    Args:
        option_array_data: The option array data from the JSON.

    Returns:
        A list of dictionaries, each representing a product option. 
        Returns an empty list if input is invalid or None.
    """
...
def trigger_bigquery_load(cloud_event):
    ...
    try:
        ...
        processed_folder = "bigdata_processed/raw_events/"
        prefix = "bigdata_importing_raw_events/"

        table = client.get_table(table_id)
        schema = table.schema
        total_inserted_records = 0
        chunk_size = 1000
        records_processed = 0  # Initialize record counter
        update_interval = 1000 # Update progress every 1000 records

        # Iterate through blobs with specified prefix.
        ...
        for blob in blobs:
            if blob.name.endswith(".json"):
                ...
                # #File moving
                destination_blob_name = processed_folder + blob.name.replace("bigdata_importing_raw_events/", "")
                try:
...
def process_data_chunk(chunk):
    ...
    for item in chunk:
        ...
        row['timestamp'] = int(item['time_stamp']['$numberInt']) if 'time_stamp' in item and '$numberInt' in item['time_stamp'] else 0
        row['ip'] = item.get('ip')
        ...
        if order_id_data is not None and isinstance(order_id_data, dict) and ('$numberInt' in order_id_data or '$numberDouble' in order_id_data):
...
def insert_rows_with_retry(client, table, rows_to_insert):
    """Inserts rows into BigQuery with retry logic."""
